...
def process_domains_screensht(domains, outfile,counts):
        from DPhelper import DPHelper
        browser=DPHelper(                
            browser_path = r"C:\Users\Administrator\AppData\Local\Google\Chrome\Bin",
            HEADLESS=False,
...
# print(domains)
outfilepath=inputfilepath.replace('.csv','-uuid.csv')
outfile = Recorder(folder_path+'/'+outfilepath, cache_size=50)
failedfilepath=inputfilepath.replace('.csv','-uuid-error.csv')
